...
TUXEDO Computer's kernel driver exposes two interfaces, sysfs for LED and backlight control and ioctl for almost everything else.
...
Yet with the latest version of the kernel driver, Tuxedo moved their interface to `/sys/class/led` which allows `tuxedo_sysfs` to pick up LED devices from other manufacturers as well.
...
Once you initialize the hardware, you either get a trait object with the supported functionality or [`None`](https://doc.rust-lang.org/std/option/enum.Option.html#variant.None) if the device doesn't support a certain features.
Basically, hardware support is perfectly matched by Rust's type system.

### Level 2 - The system service

With the first layer completed, the next step was to build a system service that's able to manage the hardware and different preferences.
The resulting `tailord` crate takes care of all the heavy lifting and exposes a simple DBUS interface for CLI or GUI applications.
...
Then, you use profiles to assign fan curves and RGB animations to your hardware.
By selecting a profile, you can easily switch between different configurations.

Compared to the TCC, `tailord` has some additional features, but also lacks certain features.
We're currently working on closing the gap, but `tailord` should already offer more than enough for most users.
Due to using Rust and better control algorithms, `tailord` was able to greatly outperform TCC's `tccd` service both in CPU and RAM usage.
...
It is well known that Rust has outstanding performance and tends to result in very robust programs.
...
Yet, it manages to be a great choice for all those tasks.
In other words, Rust simply accumulates a lot of great features that you would miss in other languages.
...
Rust's type system and safety guarantees were very valuable as well.
For example, `tuxedo_ioctl` uses a simple abstraction that makes it impossible to read write-only values and vice versa at compile time.
Additionally, as mentioned earlier, Rust made it possible to represent hardware capabilities through traits which prevents any potential misuse.
Also, it sounds like a good idea for me to write a system service that runs on elevated permissions in a memory safe language to reduce the attack surface.

On top of that, Rust also has a great ecosystem of crates that provides all we need.
[`nix`](https://crates.io/crates/nix) offers great abstractions over ioctl and [`tokio_uring`](https://crates.io/crates/tokio_uring) made it possible to access the sysfs interfaces asynchronously through [`io_uring`](https://kernel.dk/io_uring.pdf).
For DBUS interfaces [`zbus`](https://crates.io/crates/zbus) is an outstanding crate for both servers and clients.
And finally, [`Relm4`](https://crates.io/crates/relm4) makes it easy to write native GUI applications and offers great async capabilities.
...
In particular, tuxedo-rs needs more testing due to the limited hardware available to the contributors.
Let us know if you want to add new features or experienced a problem.
Contributions are always highly appreciated!

## Credit

I want to thank TUXEDO Computers for offering great Linux hardware and publishing the source code of their components.
